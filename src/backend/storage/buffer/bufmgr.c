						...
						uint32 *extended_by)
{
	...
	for (uint32 i = 0; i < extend_by; i++)
	{
		...
		TerminateSharedBufferIO(buf_hdr, /* syncio = */ true,
								/* clear_dirty = */ false,
								BM_VALID,
								/* forget_owner = */ true);
	}
			...
			IOContext io_context)
{
	...
	if (!io_data_force_async)
	{
		 ...
		 */
		TerminateSharedBufferIO(buf, /* syncio = */ true,
								/* clear_dirty = */ true,
								0,
								/* forget_owner = */ true);
	}
 ...
 */
static void
TerminateSharedBufferIO(BufferDesc *buf, bool syncio, bool clear_dirty,
						uint32 set_flag_bits, bool forget_owner)
{
	...
	/* FIXME: double condition shouldn't be necessary */
	if (forget_owner && syncio)
