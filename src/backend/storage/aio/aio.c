...
AioShmemInit(void)
{
	...
	if (!found)
	{
		...
		for (int procno = 0; procno < TotalProcs; procno++)
		{
			...
			dclist_init(&bs->unused);
			dclist_init(&bs->outstanding);
			dclist_init(&bs->pending);
			dclist_init(&bs->issued);
			dclist_init(&bs->issued_abandoned);
			pg_atomic_init_u32(&bs->inflight_count, 0);
			dlist_init(&bs->reaped);

			dclist_init(&bs->foreign_completed);
...
pgaio_postmaster_before_child_exit(int code, Datum arg)
{
	...
	while (!dclist_is_empty(&my_aio->outstanding))
	{
		PgAioInProgress *io = dclist_head_element(PgAioInProgress, owner_node, &my_aio->outstanding);
	...
	while (!dclist_is_empty(&my_aio->issued))
	{
		PgAioInProgress *io = dclist_head_element(PgAioInProgress, owner_node, &my_aio->issued);
...
{
	/* FIXME: handle unused */
	Assert(dclist_is_empty(&my_aio->outstanding));

	Assert(dclist_is_empty(&my_aio->pending));

	Assert(dclist_is_empty(&my_aio->issued));

	Assert(dclist_is_empty(&my_aio->issued_abandoned));

	Assert(pg_atomic_read_u32(&my_aio->inflight_count) == 0);

	Assert(dlist_is_empty(&my_aio->reaped));

	Assert(dclist_is_empty(&my_aio->local_completed));

	Assert(dclist_is_empty(&my_aio->foreign_completed));
...
pgaio_postmaster_child_init(void)
{
	...
	Assert(dclist_count(&my_aio->unused) == 0);
	Assert(dclist_is_empty(&my_aio->unused));
	Assert(dclist_is_empty(&my_aio->outstanding));
	Assert(dclist_is_empty(&my_aio->issued));
	Assert(dclist_is_empty(&my_aio->issued_abandoned));
	Assert(dclist_is_empty(&my_aio->pending));
...
{
	int			pending_count_before = dclist_count(&my_aio->pending);
	...
	if (pending_count_before != dclist_count(&my_aio->pending))
		pgaio_submit_pending(false);
		...
		dlist_foreach_modify(iter, &local_recycle_requests)
		{
			...
			if (cur->user_referenced)
			{
				...
				if (cur->owner_id != my_aio_id)
				{
					...
					dclist_push_tail(&other->foreign_completed, &cur->io_node);
				...
				else
				{
					...
					dclist_push_tail(&my_aio->local_completed, &cur->io_node);
				}
			}
			else
			{
				...
				dclist_delete_from(&other->issued_abandoned, &cur->owner_node);
...
{
	if (!dclist_is_empty(&my_aio->local_completed))
	{
		/*
		...
		while (!dclist_is_empty(&my_aio->local_completed))
		{
			dlist_node *node = dclist_head_node(&my_aio->local_completed);
			PgAioInProgress *io = dlist_container(PgAioInProgress, io_node, node);
...
pgaio_transfer_foreign_to_local(void)
{
	 ...
	 *
	 * NB: Can't use dclist_is_empty() at first, it cross-checks guarantees
	 * that don't hold without the spinlock held.
	 */
	if (my_aio->foreign_completed.count > 0)
	{
		SpinLockAcquire(&my_aio->foreign_completed_lock);

		while (!dclist_is_empty(&my_aio->foreign_completed))
		{
			dlist_node *node = dclist_pop_head_node(&my_aio->foreign_completed);
			PgAioInProgress *io = dlist_container(PgAioInProgress, io_node, node);

			Assert(!(io->flags & PGAIOIP_LOCAL_CALLBACK_CALLED));

			dclist_push_tail(&my_aio->local_completed, &io->io_node);
...
pgaio_synchronous_submit(void)
{
	...
	while (!dclist_is_empty(&my_aio->pending))
	{
		dlist_node *node;
		PgAioInProgress *io;

		node = dclist_head_node(&my_aio->pending);
...
pgaio_submit_pending_internal(bool drain, bool call_shared, bool call_local, bool will_wait)
{
	...
	if (dclist_is_empty(&my_aio->pending))
	{
		return;
	}
	...
	orig_total = dclist_count(&my_aio->pending);
...
#define COMBINE_ENABLED
...
#endif
	if (dclist_count(&my_aio->pending) > 1 && PGAIO_MAX_COMBINE > 1)
	...
	while (!dclist_is_empty(&my_aio->pending))
	{
		...
		pending_count = dclist_count(&my_aio->pending);
...
{
	if (limit_to <= dclist_count(&my_aio->pending))
...
pgaio_apply_backend_limit(void)
{
	...
	while (current_inflight >= io_max_concurrency)
	{
		...
		if (!dclist_is_empty(&my_aio->issued))
		{
			dlist_iter	iter;

			dclist_foreach(iter, &my_aio->issued)
			{
				...
				if (io->flags & PGAIOIP_INFLIGHT)
				{
					...
					ereport(DEBUG2,
							errmsg("applying per-backend limit to issued IO %zu/%llu (current %d in %d, target %d)",
								   ...
								   dclist_count(&my_aio->issued) + dclist_count(&my_aio->issued_abandoned),
								   current_inflight,
								   io_max_concurrency),
							errhidestmt(true),
							errhidecontext(true));
		...
		if (!dclist_is_empty(&my_aio->issued_abandoned))
		{
			dlist_iter	iter;
			PgAioIoRef	ref;
			...
			LWLockAcquire(SharedAIOCtlLock, LW_EXCLUSIVE);
			dclist_foreach(iter, &my_aio->issued_abandoned)
			{
				io = dlist_container(PgAioInProgress, owner_node, iter.cur);
...
pgaio_wait_for_issued(void)
{
	...
	while (!dclist_is_empty(&my_aio->issued_abandoned))
	{
		...
		if (!dclist_is_empty(&my_aio->issued_abandoned))
		{
			io = dclist_head_element(PgAioInProgress, owner_node, &my_aio->issued_abandoned);
			pgaio_io_ref_internal(io, &ref);
		}
		LWLockRelease(SharedAIOCtlLock);
...
pgaio_io_get(void)
{
	...
	/* FIXME: relax? */
	Assert(dclist_count(&my_aio->pending) < PGAIO_SUBMIT_BATCH_SIZE);
	...
	dclist_push_tail(&my_aio->outstanding, &io->owner_node);
...
pgaio_io_release(PgAioInProgress *io)
{
					 ...
					 PGAIOIP_LOCAL_CALLBACK_CALLED))
	{
		...
		dclist_delete_from(&my_aio->outstanding, &io->owner_node);
	...
	{
		dclist_delete_from(&my_aio->issued, &io->owner_node);
		...
		if (io->system_referenced)
		{
			...
			dclist_push_tail(&my_aio->issued_abandoned, &io->owner_node);
		}
		else
		{
	...
	if (!io->system_referenced)
	{
		...
		if (io->flags & PGAIOIP_DONE)
		{
			if (io->flags & PGAIOIP_FOREIGN_DONE)
			{
				...
				dclist_delete_from(&my_aio->foreign_completed, &io->io_node);
				SpinLockRelease(&my_aio->foreign_completed_lock);
			}
			else if (!(io->flags & PGAIOIP_LOCAL_CALLBACK_CALLED))
			{
				dclist_delete_from(&my_aio->local_completed, &io->io_node);
...
pgaio_io_prepare(PgAioInProgress *io, PgAioOp op)
{
	...
	if (dclist_count(&my_aio->pending) + 1 >= PGAIO_SUBMIT_BATCH_SIZE)
	{
#ifdef PGAIO_VERBOSE
		ereport(DEBUG3,
...
pgaio_io_stage(PgAioInProgress *io, PgAioSharedCallback scb)
{
	...
	dclist_push_tail(&my_aio->pending, &io->io_node);
...
pgaio_io_prepare_submit(PgAioInProgress *io, uint32 ring)
{
	...
	while (true)
	{
		...
		dclist_delete_from(&my_aio->pending, &cur->io_node);

		if (cur->user_referenced)
		{
			dclist_delete_from(&my_aio->outstanding, &cur->owner_node);

			dclist_push_tail(&my_aio->issued, &cur->owner_node);
		}
		else
		{
#ifdef PGAIO_VERBOSE
			...
			LWLockAcquire(SharedAIOCtlLock, LW_EXCLUSIVE);
			dclist_push_tail(&my_aio->issued_abandoned, &cur->owner_node);
			LWLockRelease(SharedAIOCtlLock);
		}
...
wait_ref_again:
	...
	while (true)
	{
		...
		if (!dclist_is_empty(&my_aio->pending) && call_shared)
		{
			/*
...
pgaio_io_retry(PgAioInProgress *io)
{
	...
	dclist_push_tail(&my_aio->issued, &io->owner_node);
...
static void
ResOwnerReleaseAioBB(Datum res)
{
	PgAioBounceBuffer *bb = (PgAioBounceBuffer *) DatumGetPointer(res);
...
static char *
ResOwnerPrintAioBB(Datum res)
{
	return psprintf("aio BB reference leak: bb %p still referenced",
					DatumGetPointer(res));
}

static const ResourceOwnerDesc aio_bb_resowner_desc =
{
	.name = "AioBB",
	.release_phase = RESOURCE_RELEASE_BEFORE_LOCKS,
	.release_priority = RELEASE_PRIO_AIO_BB,
	.ReleaseResource = ResOwnerReleaseAioBB,
	.DebugPrint = ResOwnerPrintAioBB
};

static inline void
ResourceOwnerRememberAioBB(ResourceOwner owner, PgAioBounceBuffer *bb)
{
	ResourceOwnerRemember(owner, PointerGetDatum(bb), &aio_bb_resowner_desc);
}

static inline void
ResourceOwnerForgetAioBB(ResourceOwner owner, PgAioBounceBuffer *bb)
{
	ResourceOwnerForget(owner, PointerGetDatum(bb), &aio_bb_resowner_desc);
}
