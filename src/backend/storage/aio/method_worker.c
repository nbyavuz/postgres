...
IoWorkerMain(const void *startup_data, size_t startup_data_len)
{
	...
	while (!ShutdownRequestPending)
	{
		...
		else
		{
			...
			/*
			 * Like the background writer, we have no transaction-end cleanup to
			 * destroy SMGR objects, so do so when idle after a checkpoint.
			 * Any IO has completed and its error context has been cleared here,
			 * so no borrowed file descriptors or SMGR references remain in use.
			 */
