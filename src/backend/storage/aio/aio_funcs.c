...
#define PG_STAT_GET_AIO_BACKEND_COLS	13
	...
	for (int i = 0; i < aio_ctl->backend_state_count; i++)
	{
		...
		values[7] = Int32GetDatum(dclist_count(&bs->unused));
		values[8] = Int32GetDatum(dclist_count(&bs->outstanding));
		values[9] = Int32GetDatum(dclist_count(&bs->pending));
		values[10] = Int32GetDatum(dclist_count(&bs->local_completed));
		values[11] = Int32GetDatum(dclist_count(&bs->foreign_completed));
