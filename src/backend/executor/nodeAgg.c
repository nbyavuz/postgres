...
static TupleHashEntryData *lookup_hash_entry(AggState *aggstate, int setno);
									...
									ExprContext *aggcontext)
{
		...
		DatumGetPointer(newVal) != DatumGetPointer(pergroupstate->transValue.value))
	{
		if (!fcinfo->isnull)
		{
			MemoryContextSwitchTo(aggcontext->ecxt_per_tuple_memory);
		...
		if (!pergroupstate->transValue.isnull)
		{
			if (DatumIsReadWriteExpandedObject(pergroupstate->transValue.value,
											   false,
											   pertrans->transtypeLen))
				DeleteExpandedObject(pergroupstate->transValue.value);
			else
				pfree(DatumGetPointer(pergroupstate->transValue.value));
		}
	}
								 ...
								 int setno)
{
							  ...
							  true, newVal, isNull, &newAbbrevVal))
	{
		...
		{
			advance_ordered_transition_function(aggstate, pertrans,
												fcinfo,
												pergroupstate,
												percall->aggcontext);
 ...
 * tuple (already set in tmpcontext's outertuple slot), in the indicated
 * grouping set.
 *
 * When called, CurrentMemoryContext should be the per-query context.
 */
static TupleHashEntryData *
lookup_hash_entry(AggState *aggstate, int setno)
{
...
lookup_hash_entries(AggState *aggstate)
{
	...
	for (setno = 0; setno < numHashes; setno++)
		pergroup[setno] = lookup_hash_entry(aggstate, setno)->additional;
}
						 ...
						 List *transnos)
{
	...
	foreach(lc, transnos)
	{
		 ...
		 */
		if (initValue->isnull && pertrans->initValue.isnull)
			return transno;

		if (!initValue->isnull && !pertrans->initValue.isnull &&
			datumIsEqual(initValue->value, pertrans->initValue.value,
...
AggStateIsShared(FunctionCallInfo fcinfo)
{
	if (fcinfo->context && IsA(fcinfo->context, AggStatePerCallContext))
	{
		...
		/* (valid when in a transition function) */
		if (percall->pertrans)
