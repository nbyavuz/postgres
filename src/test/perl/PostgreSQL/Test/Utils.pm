...
{
	my ($extra_test) = @_;
	# Set extra tests that treat the loopback interface as a private
	...
	# Set tests categories
	my %test_categories = (
		'needs_private_lo' => {%needs_private_lo},
	);

	# Look for if the individual extra test is enabled
	if ($ENV{PG_TEST_EXTRA} =~ m/\b$extra_test\b/)
	{
		return 1;
	}
	# Traverse the test categories, look for both if the test catagory
	# is enabled and extra test exists under this category
	else
	{
